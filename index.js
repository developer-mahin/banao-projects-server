...
async function run() {
    ...
    const chatCollection = client.db("banao-project").collection("chats")
    const jobCollection = client.db("banao-project").collection("jobs")
    ...
    try {
        ...
        //get user for search query 
        app.get("/serach-user", verifyJWT, async (req, res) => {
            ...
            const users = await usersCollection.find(keyword).toArray()
            res.send(users)
        ...
        */
        ...
        app.get("/getASingleJob/:id", async (req, res) => {
            const { id } = req.params
            const query = { _id: new ObjectId(id) }
        ...
        app.post("/applyJob", verifyJWT, async (req, res) => {
            ...
            const result = await jobCollection.updateOne(query, option)
            res.send(result)
        })
    }
